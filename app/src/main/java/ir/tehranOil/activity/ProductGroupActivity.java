...
import ir.tehranOil.adapter.AdapterJobCategory;
import ir.tehranOil.adapter.AdapterListProduceGroup;
import ir.tehranOil.app.DataBaseHelper;
import ir.tehranOil.app.G;
import ir.tehranOil.app.PreferenceUtil;
import ir.tehranOil.app.RecyclerItemClickListener;
import ir.tehranOil.model.ModelJobCategory;
import ir.tehranOil.model.SliderItem;
import ir.tehranOil.model.dbModel.ModelProduceGroup;
...
//                        ids_ServiceAvailable = ids_ServiceAvailable.substring(0,ids_ServiceAvailable.length()-1);
            ...
            public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
                try {
                    ...
                    if (records.length() > 0) {
                        for (int i = 0; i < records.length(); i++) {
                            JSONObject obj = records.getJSONObject(i);
                            Call<ResponseBody> request = api.deleteProductGroupAvailable(obj.getInt("id") + "");
                            request.enqueue(new retrofit2.Callback<ResponseBody>() {
                                @Override
                                public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
                                    String result = G.getResult(response);
                                    G.Log(call.request().toString());
                                    G.Log(result);
                                    addServiceProductGroup(objectSend);
                                }
    ...
    public void addServiceProductGroup(JSONObject object) {
        G.Log(object + "");
        Api api = RetrofitClient.createService(Api.class, G.api_username, G.api_password);
...
//        String d_id = PreferenceUtil.getD_id();
        ...
        try {
            ...
            object.put("km_usage", 0);
            object.put("job_category_id", jobid + "");
            object.put("change_wage", "");
            object.put("send_msg", 0);
            object.put("status", 0);
        ...
        Call<ResponseBody> request = api.addProductGroup(G.returnBody(object.toString()));
        countxx++;
        request.enqueue(new retrofit2.Callback<ResponseBody>() {
            @Override
            public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
